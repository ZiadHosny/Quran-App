...
export const saveUserProgress = catchAsyncError(
  async (req: AuthRequest, res: Response) => {
    const { body, user } = req;
    const { currentSurah, currentMin, random, repeat, volume, quranReciterId } =
      body as UserProgress;
...
export const addSurahToUserPlaylist = catchAsyncError(
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    const { user, body } = req;
...
export const removeSurahToUserPlaylist = catchAsyncError(
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    const { user, body } = req;
...
// GET User Progress
export const getUserPlaylist = catchAsyncError(
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    const { user } = req;
